...
**JWT (необязательно).** Если сервер запущен с `-jwt-secret`, вместо
статического токена можно прислать подписанный HS256-токен в том же
заголовке `Authorization: Bearer`:

```sh
export WEBDB_JWT_SECRET=...            # на сервере и у того, кто выпускает токены
./webdb -data ./data -jwt-issuer tinydb -jwt-audience api
./webdb token -sub alice -ttl 24h -iss tinydb -aud api     # выдать токен
```

Сервер проверяет подпись, `exp`/`nbf` и — если заданы флагами — `iss`/`aud`.
Алгоритм берётся из конфига сервера, а не из заголовка токена, поэтому
`alg: none` и подмена RS256/HS256 отвергаются. Статический токен при включённом
JWT продолжает работать: они не исключают, а дополняют друг друга.
